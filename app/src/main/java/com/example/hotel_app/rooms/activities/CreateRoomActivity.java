    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        button.setOnClickListener(v -> {
            if (!number.getText().toString().isEmpty() && !beds.getText().toString().isEmpty() && !price.getText().toString().isEmpty() && !kind.toString().equals("Nothing")) {
                APIService.APIService.createRoom(new Room(Integer.parseInt(number.getText().toString()), kind.toString(), Integer.parseInt(beds.getText().toString()), Integer.parseInt(price.getText().toString())), new Callback() {
                    @Override
                        ...
                        if (!base64Image.toString().isEmpty()) {
                            APIService.APIService.postPhoto(kind.toString(), base64Image.toString(), new Callback() {
                                ...
                                public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                                    call.cancel();
                                    runOnUiThread(() -> {
                                        Toast.makeText(getApplicationContext(), "Room has been created", Toast.LENGTH_SHORT).show();
                                        finish();
                                    });
                                }
                            });
